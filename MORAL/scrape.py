...
def parse_log_file(log_file_path: str) -> Dict[str, List[Dict]]:
    ...
    ram_pattern = re.compile(
        r"CPU: [\d.]+% \| RAM: ([\d.]+)GB \| GPU: ([\d.]+)% \| GPU-MEM: ([\d.]+)/([\d.]+)GB"
    )
    ...
    for i, line in enumerate(lines):
        ...
        if run_match:
            ...
            if time_match:
                try:
                    run_data["start_time"] = datetime.strptime(
        ...
        if epoch_match and run_data:
            ...
            run_data["epochs"].append(
                {"epoch": epoch_num, "train_loss": train_loss, "valid_loss": valid_loss}
            )
        ...
        if "Finished run" in line and run_data:
            ...
            if time_match:
                try:
                    run_data["end_time"] = datetime.strptime(
                        time_match.group(1), "%Y-%m-%d %H:%M:%S.%f"
                    )
                except Exception:
                    pass
...
def compute_statistics(datasets: Dict[str, List[Dict]]) -> Dict[str, Dict]:
    ...
    for dataset, runs in datasets.items():
        ...
        for run in runs:
            ...
            if run["ram_usage"]:
                ...
                gpu_mem_percent_values = [
                    m["gpu_mem_percent"] for m in run["ram_usage"]
                ]
        ...
        gpu_mem_stats = {
            ...
            "std_used": (
                statistics.stdev(all_gpu_mem_used) if len(all_gpu_mem_used) > 1 else 0
            ),
            "min_used": min(all_gpu_mem_used) if all_gpu_mem_used else 0,
            "max_used": max(all_gpu_mem_used) if all_gpu_mem_used else 0,
            "avg_percent": (
                statistics.mean(all_gpu_mem_percent) if all_gpu_mem_percent else 0
            ),
            "median_percent": (
                statistics.median(all_gpu_mem_percent) if all_gpu_mem_percent else 0
            ),
        }
        ...
        stats[dataset] = {
            "num_runs": len(runs),
            "avg_epochs_per_run": (
                statistics.mean(total_epochs_per_run) if total_epochs_per_run else 0
            ),
            "std_epochs_per_run": (
                statistics.stdev(total_epochs_per_run)
                if len(total_epochs_per_run) > 1
                else 0
            ),
            "avg_early_stop_epoch": (
                statistics.mean(early_stop_epochs) if early_stop_epochs else 0
            ),
            "avg_best_epoch": statistics.mean(best_epochs) if best_epochs else 0,
            "avg_final_valid_loss": (
                statistics.mean(final_valid_losses) if final_valid_losses else 0
            ),
            "std_final_valid_loss": (
                statistics.stdev(final_valid_losses)
                if len(final_valid_losses) > 1
                else 0
            ),
            "avg_epoch_duration_seconds": (
                statistics.mean(run_durations) if run_durations else 0
            ),
            "std_epoch_duration_seconds": (
            ...
            "avg_total_run_time_seconds": (
                statistics.mean(total_times) if total_times else 0
            ),
...
def print_statistics(stats: Dict[str, Dict]):
    ...
    for dataset, dataset_stats in stats.items():
        ...
        print(
            f"  Average epochs per run: {dataset_stats['avg_epochs_per_run']:.2f} ± {dataset_stats['std_epochs_per_run']:.2f}"
        )
        print(
            f"  Average early stopping epoch: {dataset_stats['avg_early_stop_epoch']:.2f}"
        )
        print(f"  Average best epoch: {dataset_stats['avg_best_epoch']:.2f}")
        print(
            f"  Average final validation loss: {dataset_stats['avg_final_valid_loss']:.6f} ± {dataset_stats['std_final_valid_loss']:.6f}"
        )

        if dataset_stats["avg_epoch_duration_seconds"] > 0:
            print(f"  Epoch duration statistics:")
            print(
                f"    Average: {dataset_stats['avg_epoch_duration_seconds']:.2f} seconds"
            )
            print(
                f"    Std dev: {dataset_stats['std_epoch_duration_seconds']:.2f} seconds"
            )
            ...
            print(
                f"    Average per hour: {3600/dataset_stats['avg_epoch_duration_seconds']:.2f} epochs/hour"
            )
            print(
                f"    Total run time: {dataset_stats['avg_total_run_time_seconds']/60:.2f} minutes (average)"
            )

        print(
            f"  RAM Usage Statistics ({dataset_stats['total_ram_measurements']} measurements):"
        )
        print(f"    CPU RAM:")
        print(f"      Average: {dataset_stats['cpu_ram_stats']['avg']:.2f} GB")
        print(f"      Std dev: {dataset_stats['cpu_ram_stats']['std']:.2f} GB")
        print(
            f"      Range: {dataset_stats['cpu_ram_stats']['min']:.2f} - {dataset_stats['cpu_ram_stats']['max']:.2f} GB"
        )
        ...
        print(
            f"      Range: {dataset_stats['gpu_util_stats']['min']:.1f} - {dataset_stats['gpu_util_stats']['max']:.1f}%"
        )
        print(f"      Median: {dataset_stats['gpu_util_stats']['median']:.1f}%")

        print(f"    GPU Memory:")
        print(
            f"      Average used: {dataset_stats['gpu_mem_stats']['avg_used']:.2f} GB"
        )
        print(
            f"      Range used: {dataset_stats['gpu_mem_stats']['min_used']:.2f} - {dataset_stats['gpu_mem_stats']['max_used']:.2f} GB"
        )
        print(
            f"      Average utilization: {dataset_stats['gpu_mem_stats']['avg_percent']:.1f}%"
        )
        print(
            f"      Median utilization: {dataset_stats['gpu_mem_stats']['median_percent']:.1f}%"
        )
...
def create_csv_summary(
    stats: Dict[str, Dict], output_file: str = "dataset_summary.csv"
):
    """
    ...
    headers = [
        ...
        "Std_Epoch_Duration_s",
        "Min_Epoch_Duration_s",
        "Max_Epoch_Duration_s",
        "Epochs_Per_Hour",
        "Avg_Total_Run_Time_min",
        "CPU_RAM_Avg_GB",
        "CPU_RAM_Std_GB",
        "CPU_RAM_Min_GB",
        "CPU_RAM_Max_GB",
    ...
    for dataset, runs in datasets.items():
        ...
        for run in runs:
            if run["ram_usage"]:
                ...
                print(
                    f"  Run {run['run_number']}: {len(run['ram_usage'])} RAM measurements"
                )
                if cpu_ram_values:
                    print(
                        f"    CPU RAM: {statistics.mean(cpu_ram_values):.2f} GB avg, "
                        f"{min(cpu_ram_values):.2f}-{max(cpu_ram_values):.2f} GB range"
                    )
                if gpu_util_values:
                    print(
                        f"    GPU Util: {statistics.mean(gpu_util_values):.1f}% avg, "
                        f"{min(gpu_util_values):.1f}-{max(gpu_util_values):.1f}% range"
                    )
                if gpu_mem_values:
                    print(
                        f"    GPU Mem: {statistics.mean(gpu_mem_values):.2f} GB avg, "
        ...
        if all_cpu_ram:
            ...
            print(
                f"    CPU RAM - Overall: {statistics.mean(all_cpu_ram):.2f} GB avg, "
                f"{statistics.stdev(all_cpu_ram):.2f} GB std"
            )
            print(
                f"    GPU Util - Overall: {statistics.mean(all_gpu_util):.1f}% avg, "
                f"{statistics.stdev(all_gpu_util):.1f}% std"
            )
            print(
                f"    GPU Mem - Overall: {statistics.mean(all_gpu_mem):.2f} GB avg, "
            ...
            if run["epoch_timestamps"]:
                ...
                if len(run["epoch_timestamps"]) >= 2:
                    ...
                    if epoch_diff > 0:
                        ...
                        print(
                            f"    Time between: {total_time:.2f} seconds for {epoch_diff} epochs"
                        )
                        print(
                            f"    Average per epoch: {avg_time_per_epoch:.2f} seconds"
                        )
...
def main():
    ...
    try:
        ...
        for dataset, runs in datasets.items():
            ...
            print(
                f"  {dataset}: {len(runs)} runs, {total_ram_measurements} RAM measurements"
            )
