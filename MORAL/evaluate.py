...
def calculate_individual_fairness(scores, edges, features, k=100, threshold=0.7):
    """Compute Individual Fairness at k."""
    ...
    edge_emb = np.concatenate([features[edges_k[:, 0]], features[edges_k[:, 1]]], axis=1)
    ...
    for ds in datasets:
        try:
            ...
            test_labels_all = torch.cat(
                [torch.ones(test_pos.size(0)), torch.zeros(test_neg.size(0))], dim=0
            ).numpy()

            test_edges_np = test_edges_all.cpu().numpy().astype(np.int64)
            test_edge_sens_groups = (
                sens_np[test_edges_np[:, 0]] + sens_np[test_edges_np[:, 1]]
            )
            ...
            files = glob.glob(
                f"{results_dir}/three_classifiers_{ds}_*_final_ranking.pt"
            )
            method_files = {}
            ...
            for method, f_list in method_files.items():
                ...
                for f in f_list:
                    try:
                        ...
                        prec_at_k = (
                            np.mean(labels_final_np[idx][:k])
                            if len(idx) >= k
                            else np.mean(labels_final_np[idx])
                        )
